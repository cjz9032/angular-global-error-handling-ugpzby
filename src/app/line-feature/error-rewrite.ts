...
Zone.__load_patch('Error', (global: any, Zone: ZoneType, api: _ZonePrivate) => {
	...
	function buildZoneAwareStackFrames(
		...
		for (; i < frames.length && zoneFrame; i++) {
			let frame = frames[i];
			if (frame.trim()) {
				if (frame.match && !frame.includes('(')) {
					const res = frame.match(/at\s(.+)$/);
					if (res) {
						frames[i] = frame = frame.replace(res[1], `(${res[1]})`);
					}
				}
	...
	const detectZone: Zone = Zone.current.fork({
		name: 'detect',
		onHandleError(parentZD: ZoneDelegate, current: Zone, target: Zone, error: any): boolean {
			if (error.originalStack && Error === ZoneAwareError) {
				const frames = error.originalStack.split(/\n/);
				let runFrame = false;
				let runGuardedFrame = false;
				let runTaskFrame = false;
				while (frames.length) {
					...
					if (/:\d+:\d+/.test(frame) || frame === 'ZoneAwareError') {
						...
						if (fnName.indexOf('ZoneAwareError') !== -1) {
							if (fnName.indexOf('new ZoneAwareError') !== -1) {
								...
								zoneAwareFrame2 = frame.replace(
									'new ZoneAwareError',
									'new Error.ZoneAwareError'
								);
							} else {
