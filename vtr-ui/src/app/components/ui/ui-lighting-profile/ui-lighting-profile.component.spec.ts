...
const gamingLightingServiceMock = jasmine.createSpyObj('GamingLightingService', [
	...
	'optionChangedRGBTop',
	'optionChangedRGBSide',
	'setLightingDefaultProfileById',
	'setLightingProfileEffectColor',
]);
const deviceServiceMock = jasmine.createSpyObj('DeviceService', [
	'isShellAvailable',
	'getMachineInfo',
]);
...
describe('UiLightingProfileComponent', () => {
	...
	it('should update the lightining features for single color', fakeAsync(() => {
		...
		gamingLightingServiceMock.getLightingCapabilities.and.returnValue(
			Promise.resolve(singleColorResponse)
		);
	...
	it('should update the lightining features for multi color', fakeAsync(() => {
		...
		fixture.detectChanges();
		gamingLightingServiceMock.getLightingCapabilities.and.returnValue(
			Promise.resolve(multipleColorResponse)
		);
		component.getGamingLightingCapabilities();
		tick(10);
		expect(Object.keys(component.lightingCapabilities).length).toBeGreaterThanOrEqual(1);
	...
	it('should call the setLightingBrightness', fakeAsync(() => {
		gamingLightingServiceMock.setLightingProfileBrightness.and.returnValue(
