	...
	public setToggleStatusCache() {
		try {
			if (this.listInfo && this.profileId !== 0) {
				...
				if (this.isDefault) {
					if (
						this.toggleStatusLNBx50['profileId' + this.profileId].defaultStatus ===
						'undefined'
					) {
						this.isDivideArea = this.gamingLightingService.checkAreaColorFn(
							this.listInfo
						);
						this.toggleStatusLNBx50[
							'profileId' + this.profileId
						].defaultStatus = this.isDivideArea;
						this.toggleStatusLNBx50[
							'profileId' + this.profileId
						].status = this.isDivideArea;
						this.localCacheService.setLocalCacheValue(
							LocalStorageKey.KeyboardToggleStatusLNBx50,
							this.toggleStatusLNBx50
						);
					} else {
						this.isDivideArea = this.toggleStatusLNBx50[
							'profileId' + this.profileId
						].defaultStatus;
						this.toggleStatusLNBx50[
							'profileId' + this.profileId
					...
					if (
						this.toggleStatusLNBx50['profileId' + this.profileId].status === 'undefined'
					) {
						this.isDivideArea = this.gamingLightingService.checkAreaColorFn(
							this.listInfo
						);
