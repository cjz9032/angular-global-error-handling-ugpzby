...
export class WidgetAddedAppListComponent implements OnInit, OnChanges {
    ...
    removeApp(app: any, i: any) {
        try {
            if (this.isNetworkBoost === true) {
                this.networkBoostService.deleteProcessInNetBoost(app.processPath).then((response: boolean) => {
                    if (response) {
                        this.getNetworkBoostList();
                    }
                });
            } else {
                this.autoCloseService.delAppsAutoCloseList(app.processDescription).then((response: boolean) => {
                    if (response) {
                        this.getAutoCloseList();
                    }
                });
    ...
    focusAddButton() {
        setTimeout(() => {
            const id = this.isNetworkBoost ? 'networkboost_addButton_clickable' : 'addAutoCloseAppBtn';
