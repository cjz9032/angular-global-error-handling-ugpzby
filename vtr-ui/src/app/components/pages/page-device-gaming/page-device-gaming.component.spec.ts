...
const dashboardServiceMock = jasmine.createSpyObj('DashboardService', ['onlineCardContent','offlineCardContent','setDefaultCMSContent','translateString']);
const dialogServiceMock = jasmine.createSpyObj('DialogService', ['onlineCardContent','offlineCardContent']);
const gamingAllCapabilitiesServiceMock = jasmine.createSpyObj('GamingAllCapabilitiesService', ['openLenovoIdDialog']);
describe('PageDeviceGamingComponent', () => {
	...
	beforeEach(async(() => {
		TestBed.configureTestingModule({
			declarations: [PageDeviceGamingComponent, SvgInlinePipe, FeedbackFormComponent,
				mockPipe({ name: 'sanitize' })],
			schemas: [NO_ERRORS_SCHEMA],
			imports: [TranslationModule, HttpClientModule, RouterTestingModule, NgbModule],
            providers: [TranslateStore, GamingAllCapabilitiesService, DialogService, DevService, CommsService
            ]
		})
        .compileComponents();
    ...
    it('#PageDeviceGamingComponent : onNotification network status offline ', () => {
        const offNotificationSpy = spyOn<any>(component, 'onNotification').withArgs(mockNetworkStatusOffline).and.callThrough();
