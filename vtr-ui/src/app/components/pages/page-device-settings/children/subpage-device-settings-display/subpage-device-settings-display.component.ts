...
export class SubpageDeviceSettingsDisplayComponent
			...
			if (this.displayService.isShellAvailable) {
				this.displayService.setEyeCareModeState(this.eyeCareModeStatus.status)
					.then((value: any) => {
						...
						this.logger.error('onEyeCareModeStatusToggle', error);
					});
			...
			if (this.displayService.isShellAvailable) {
				this.displayService.setEyeCareModeState(value)
					.then((result: boolean) => {
						...
						this.logger.error('setEyeCareModeState', error);
					});
			}
		} catch (error) {
			this.logger.error(error.message);
	...
	private getEyeCareModeStatus() {
		if (this.displayService.isShellAvailable) {
			this.displayService
				...
				.catch(error => {
					this.logger.error('getEyeCareModeState', error);
				});
		}
	}
			...
			if (this.displayService.isShellAvailable) {
				this.displayService
					...
					then((response: any) => {
						...
						this.logger.error('setEyeCareAutoMode', error);
					});
			}
		} catch (error) {
			this.logger.error(error.message);
			...
			if (this.displayService.isShellAvailable) {
				this.displayService
					...
					.then((status: SunsetToSunriseStatus) => {
						...
						this.logger.error('getSunsetToSunrise', error);
					});
			}
		} catch (error) {
			this.logger.error(error.message);
		...
		if (this.displayService.isShellAvailable) {
			this.displayService.setCameraPrivacyModeState($event.switchValue)
				.then((value: boolean) => {
					...
					this.logger.error('setCameraStatus', error);
				});
		}
	}
	//  TROUBLE
			...
			if (this.displayService.isShellAvailable) {
				this.displayService.startCameraPrivacyMonitor(this.startMonitorHandlerForCamera.bind(this))
					.then((val) => {
						...
						this.logger.error('startCameraPrivacyMonitor', error);
					});
			}
		} catch (error) {
	...
	public getPrivacyGuardCapabilityStatus() {
		this.displayService.getPrivacyGuardCapability().then((status: boolean) => {
			...
			.catch(error => {
				this.logger.error('privacy guard compatability error here', error);
			});
	...
	public getPrivacyToggleStatus() {
		this.displayService.getPrivacyGuardStatus().then((value: boolean) => {
			...
			.catch(error => {
				this.logger.error('privacy guard status error here', error);
			});
	}
		...
		this.displayService.getPrivacyGuardOnPasswordCapability().then((status: boolean) => {
			...
			if (status) {
				...
				this.displayService.getPrivacyGuardOnPasswordStatus().then((value: boolean) => {
					...
					.catch(error => {
						this.logger.error('privacy guard on password status error here', error);
					});
			}
		})
			.catch(error => {
				this.logger.error('privacy guard on password compatability error here', error);
			});
	}
	...
	public setPrivacyGuardToggleStatus(event) {
		this.displayService.setPrivacyGuardStatus(event.switchValue).then((response: boolean) => {
			...
			.catch(error => {
				this.logger.error('set privacy guard status error here', error);
			});
	}
	...
	public setPrivacyGuardOnPasswordStatusVal(event) {
		this.displayService.setPrivacyGuardOnPasswordStatus(event.target.checked).then((response: boolean) => {
			...
			.catch(error => {
				this.logger.error('set privacy guard on password status error here', error);
			});
	}
			...
			if (this.displayService.isShellAvailable) {
				this.displayService.getOLEDPowerControlCapability()
					.then((result: boolean) => {
						...
						this.logger.error('getOLEDPowerControlCapability', error);
