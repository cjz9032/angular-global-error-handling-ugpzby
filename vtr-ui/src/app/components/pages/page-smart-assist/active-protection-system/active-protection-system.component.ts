...
export class ActiveProtectionSystemComponent implements OnInit {
	...
	initAPS() {
		this.smartAssist
			...
			.then((res) => {
				...
				this.smartAssist.getAPSSensitivityLevel().then((sensitivityLevel) => {
				...
				this.smartAssist.getAutoDisableSetting().then((autoDisableSetting) => {
					this.repeatShock = autoDisableSetting;
				});
				this.smartAssist.getSnoozeSetting().then((snoozeSetting) => {
					this.manualSnooze = snoozeSetting;
					this.smartAssist.getSnoozeTime().then((snoozeTime) => {
	...
	checkAdvance() {
		Promise.all([
			this.smartAssist.getPenCapability(),
			this.smartAssist.getTouchCapability(),
			this.smartAssist.getPSensorCapability(),
		])
			.then((res: any[]) => {
	...
	setAPSMode(event = null) {
		...
		this.smartAssist.setAPSMode(value).then((res) => {});
	...
	setAPSSensitivityLevel($event: number) {
		...
		this.smartAssist.setAPSSensitivityLevel(value).then((res) => {});
	}

	setAutoDisableSetting(event) {
		this.repeatShock = event;
		this.smartAssist.setAutoDisableSetting(event).then((res) => {});
		this.commonMetricsService.sendMetrics(
			event,
			'cb.aps-auto-disable',
			CommonMetricsModel.ParentDeviceSettings
	...
	setSnoozeSetting(event) {
		...
		this.smartAssist.setSnoozeSetting(value).then((res) => {
			this.smartAssist.getSnoozeTime().then((time) => {
	...
	setSnoozeTime(event: DropDownInterval) {
		...
		this.smartAssist.setSnoozeTime(event.value.toString()).then((res) => {
			this.smartAssist.getSnoozeTime().then((time) => {
				this.manualSnoozeTime = +time;
			});
		});
	...
	suspendNow() {
		...
		this.smartAssist.sendSnoozeCommand(timeInSec).then((res) => {});
	}
