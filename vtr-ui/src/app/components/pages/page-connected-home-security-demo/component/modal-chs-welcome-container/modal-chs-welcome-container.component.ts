...
@Component({
	selector: 'vtr-modal-chs-welcome-container-demo',
	templateUrl: './modal-chs-welcome-container.component.html',
	styleUrls: ['./modal-chs-welcome-container.component.scss']
})
export class ModalChsWelcomeContainerDemoComponent implements OnInit, AfterViewInit, OnDestroy {
	...
	next(switchPage, isLenovoIdLogin, isLocationServiceOn) {
		...
		if (switchPage === 1) {
			...
			if (isLenovoIdLogin && this.chs.account.state === CHSAccountState.local && this.isOnline && this.needTrial) {
				this.startTrial();
			} else {
				this.closeModal('success');
			}
