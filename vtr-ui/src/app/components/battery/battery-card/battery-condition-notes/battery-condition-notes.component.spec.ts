...
describe('BatteryConditionNotesComponent', () => {
	...
	const acConnectedCondition = new BatteryCondition(
		Conditions.FullACAdapterSupport,
		Status.AcAdapterStatus
	);
	const goodAndAcConnectedConditions = [goodCondition, acConnectedCondition];

	const acBatteryNotDetectedCondition = new BatteryCondition(
		Conditions.NotDetected,
		Status.AcAdapterStatus
	);
	...
	describe('with a battery with store limitation', () => {
		it('and poor status should show store limitation note', () => {
			const poorAndStoreLimitation = new BatteryCondition(
				Conditions.StoreLimitation,
				Status.Poor
			);
		...
		it('and fair status should show store limitation note', () => {
			const fairAndStoreLimitation = new BatteryCondition(
				Conditions.StoreLimitation,
				Status.Fair
			);
		...
		it('and good status should not show store limitation note', () => {
			const goodAndStoreLimitation = new BatteryCondition(
				Conditions.StoreLimitation,
				Status.Good
			);
	...
	describe('with ac adapter connected', () => {
		...
		describe('and battery is not detected', () => {
			it('should render battery is not detected message', () => {
				...
				expect(h6.innerHTML).toBe(
					'device.deviceSettings.batteryGauge.condition.NotDetected'
				);
			});
