...
describe('ModalGamingThermalMode2Component', () => {
	...
	describe('thermalMode', () => {
		beforeEach(
			waitForAsync(() => {
				...
				TestBed.configureTestingModule({
					...
					imports: [TranslationModule, HttpClientModule, MatDialogModule],
					providers: [
		...
		it('getThermalModeSettingStatus', fakeAsync(() => {
			...
			tick();
			expect(component.thermalModeSettingStatus).toBe(
				1,
				'component.thermalModeSettingStatus shoulde be 1'
			);
			expect(thermalModeSettingStatusCache).toBe(
				1,
				'thermalModeSettingStatusCache shoulde be 1'
			);
			...
			expect(component.thermalModeSettingStatus).toBe(
				2,
				'component.thermalModeSettingStatus shoulde be 2'
			);
			expect(thermalModeSettingStatusCache).toBe(
				2,
				'thermalModeSettingStatusCache shoulde be 2'
			);
			...
			expect(component.thermalModeSettingStatus).toBe(
				3,
				'component.thermalModeSettingStatus shoulde be 3'
			);
			expect(thermalModeSettingStatusCache).toBe(
				3,
				'thermalModeSettingStatusCache shoulde be 3'
			);
		}));
		it('setThermalModeSettingStatus', fakeAsync(() => {
			...
			for (let i = 1; i < 4; i++) {
				...
				for (let j = 1; j < 4; j++) {
					...
					expect(component.thermalModeSettingStatus).toBe(
						j,
						`setReturnValue is ${setReturnValue}, component.thermalModeSettingStatusCache should be ${j}`
					);
					expect(thermalModeSettingStatusCache).toBe(
						j,
						`setReturnValue is ${setReturnValue}, thermalModeSettingStatusCache should be ${j}`
					);
					expect(thermalModeSettingStatus).toBe(
						j,
						`setReturnValue is ${setReturnValue}, thermalModeSettingStatus should be ${j}`
					);
				}
			}
			...
			for (let i = 1; i < 4; i++) {
				...
				for (let j = 1; j < 4; j++) {
					...
					expect(component.thermalModeSettingStatus).toBe(
						i,
						`setReturnValue is ${setReturnValue}, component.thermalModeSettingStatus should keep ${i}`
					);
					expect(thermalModeSettingStatusCache).toBe(
						i,
						`setReturnValue is ${setReturnValue}, thermalModeSettingStatusCache should keep ${i}`
					);
					expect(thermalModeSettingStatus).toBe(
						i,
						`setReturnValue is ${setReturnValue}, thermalModeSettingStatus should keep ${i}`
					);
				}
			}
		}));
		it('getAutoSwitchStatus', fakeAsync(() => {
			...
			tick();
			expect(component.autoSwitchStatus).toBe(
				false,
				'component.autoSwitchStatus shoulde be false'
			);
			...
			expect(component.autoSwitchStatus).toBe(
				true,
				'component.autoSwitchStatus shoulde be true'
			);
		...
		it('setAutoSwitchStatus', fakeAsync(() => {
			...
			expect(component.autoSwitchStatus).toBe(
				false,
				`setReturnValue is ${setReturnValue}, component.autoSwitchStatus should be false`
			);
			expect(autoSwitchStatusCache).toBe(
				false,
				`setReturnValue is ${setReturnValue}, autoSwitchStatusCache should be false`
			);
			expect(autoSwitchStatus).toBe(
				false,
				`setReturnValue is ${setReturnValue}, autoSwitchStatus should be false`
			);
			component.setAutoSwitchStatus(true);
			tick();
			expect(component.autoSwitchStatus).toBe(
				true,
				`setReturnValue is ${setReturnValue}, component.autoSwitchStatus should be true`
			);
			expect(autoSwitchStatusCache).toBe(
				true,
				`setReturnValue is ${setReturnValue}, autoSwitchStatusCache should be true`
			);
			expect(autoSwitchStatus).toBe(
				true,
				`setReturnValue is ${setReturnValue}, autoSwitchStatus should be true`
			);
			setReturnValue = false;
			...
			expect(component.autoSwitchStatus).toBe(
				true,
				`setReturnValue is ${setReturnValue}, component.autoSwitchStatus should keep true`
			);
			expect(autoSwitchStatusCache).toBe(
				true,
				`setReturnValue is ${setReturnValue}, autoSwitchStatusCache should keep true`
			);
			expect(autoSwitchStatus).toBe(
				true,
				`setReturnValue is ${setReturnValue}, autoSwitchStatus should keep true`
			);
			...
			expect(component.autoSwitchStatus).toBe(
				false,
				`setReturnValue is ${setReturnValue}, component.autoSwitchStatus should keep false`
			);
			expect(autoSwitchStatusCache).toBe(
				false,
				`setReturnValue is ${setReturnValue}, autoSwitchStatusCache should keep false`
			);
			expect(autoSwitchStatus).toBe(
				false,
				`setReturnValue is ${setReturnValue}, autoSwitchStatus should keep false`
			);
		}));
		...
		it('onRegThermalModeChangeEvent', () => {
			for (let i = 1; i < 4; i++) {
				...
				expect(component.thermalModeSettingStatus).toBe(
					i,
					`callback is undefined, component.thermalModeSettingStatusCache should be ${i}`
				);
				expect(thermalModeSettingStatusCache).toBe(
					i,
					`callback is undefined, thermalModeSettingStatusCache should be ${i}`
				);
				for (let j = 1; j < 4; j++) {
					thermalModeSettingStatus = j;
					component.onRegThermalModeChangeEvent(j);
					expect(component.thermalModeSettingStatus).toBe(
						thermalModeSettingStatus,
						`callback is not undefined, component.thermalModeSettingStatusCache should be ${j}`
					);
					expect(thermalModeSettingStatusCache).toBe(
						thermalModeSettingStatus,
						`callback is not undefined, thermalModeSettingStatusCache should be ${j}`
					);
				}
			}
		});
		...
		it('getAutoAdjustSettingStatus', fakeAsync(() => {
			...
			component.ngOnInit();
			tick();
			expect(component.autoAdjustSettings).toBe(
				true,
				`component.isAutoAdjustSetted is false, component.autoAdjustSettings shoulde be ture`
			);
			expect(autoAdjustSettingStatusCache).toBe(
				true,
				`component.isAutoAdjustSetted is false, autoAdjustSettingStatusCache shoulde be ture`
			);
			...
			expect(component.autoAdjustSettings).toBe(
				false,
				`component.isAutoAdjustSetted is false, component.autoAdjustSettings shoulde be false`
			);
			expect(autoAdjustSettingStatusCache).toBe(
				false,
				`component.isAutoAdjustSetted is false, autoAdjustSettingStatusCache shoulde be false`
			);
			...
			expect(component.autoAdjustSettings).toBe(
				false,
				`component.isAutoAdjustSetted is true, component.autoAdjustSettings shoulde keep false`
			);
			expect(autoAdjustSettingStatusCache).toBe(
				false,
				`component.isAutoAdjustSetted is true, autoAdjustSettingStatusCache shoulde keep false`
			);
			...
			expect(component.autoAdjustSettings).toBe(
				true,
				`component.isAutoAdjustSetted is true, component.autoAdjustSettings shoulde keep true`
			);
			expect(autoAdjustSettingStatusCache).toBe(
				true,
				`component.isAutoAdjustSetted is true, autoAdjustSettingStatusCache shoulde keep true`
			);
		}));
		it('setAutoAdujustSettingStatus', fakeAsync(() => {
			...
			tick();
			expect(component.isAutoAdjustSetted).toBe(
				true,
				`setReturnValue is ${setReturnValue}, component.isAutoAdjustSetted should be true`
			);
			expect(component.autoAdjustSettings).toBe(
				true,
				`setReturnValue is ${setReturnValue}, component.autoAdjustSettings should be true`
			);
			expect(autoAdjustSettingStatusCache).toBe(
				true,
				`setReturnValue is ${setReturnValue}, autoAdjustSettingStatusCache should be true`
			);
			expect(autoAdjustSettingStatus).toBe(
				true,
				`setReturnValue is ${setReturnValue}, autoAdjustSettingStatus should be true`
			);
			component.isAutoAdjustSetted = false;
			autoAdjustSettingStatus = true;
			...
			expect(component.autoAdjustSettings).toBe(
				false,
				`setReturnValue is ${setReturnValue}, component.autoAdjustSettings should be false`
			);
			expect(autoAdjustSettingStatusCache).toBe(
				false,
				`setReturnValue is ${setReturnValue}, autoAdjustSettingStatusCache should be false`
			);
			expect(autoAdjustSettingStatus).toBe(
				false,
				`setReturnValue is ${setReturnValue}, autoAdjustSettingStatus should be false`
			);
			...
			expect(component.autoAdjustSettings).toBe(
				false,
				`setReturnValue is ${setReturnValue}, component.autoAdjustSettings should keep false`
			);
			expect(autoAdjustSettingStatusCache).toBe(
				false,
				`setReturnValue is ${setReturnValue}, autoAdjustSettingStatusCache should keep false`
			);
			expect(autoAdjustSettingStatus).toBe(
				false,
				`setReturnValue is ${setReturnValue}, autoAdjustSettingStatus should keep false`
			);
			component.isAutoAdjustSetted = false;
			component.autoAdjustSettings = true;
			autoAdjustSettingStatusCache = true;
			...
			expect(component.autoAdjustSettings).toBe(
				true,
				`setReturnValue is ${setReturnValue}, component.autoAdjustSettings should keep true`
			);
			expect(autoAdjustSettingStatusCache).toBe(
				true,
				`setReturnValue is ${setReturnValue}, autoAdjustSettingStatusCache should keep true`
			);
			expect(autoAdjustSettingStatus).toBe(
				true,
				`setReturnValue is ${setReturnValue}, autoAdjustSettingStatus should keep true`
			);
		}));
	});
	describe('performanceOC', () => {
		...
		it('setPerformanceOC(cpu&gpu)', fakeAsync(() => {
			...
			expect(cpuOCStatusCache).toBe(
				1,
				`setReturnValue is ${setReturnValue},cpuOCStatusCache should be 1(true)`
			);
			...
			expect(cpuOCStatusCache).toBe(
				3,
				`setReturnValue is ${setReturnValue},cpuOCStatusCache should be 3(false)`
			);
			expect(gpuOCStatusCache).toBe(
				3,
				`setReturnValue is ${setReturnValue},gpuOCStatusCache should be 3(false)`
			);
			...
			expect(cpuOCStatusCache).toBe(
				3,
				`setReturnValue is ${setReturnValue},cpuOCStatusCache should keep 3(false)`
			);
			expect(gpuOCStatusCache).toBe(
				3,
			...
			expect(cpuOCStatusCache).toBe(
				1,
				`setReturnValue is ${setReturnValue},cpuOCStatusCache should keep 1(true)`
			);
			expect(gpuOCStatusCache).toBe(
				1,
				`setReturnValue is ${setReturnValue},gpuOCStatusCache should keep 1(true)`
			);
		}));
		...
		it('driver lack', () => {
			...
			component.renderOCSupported();
			expect(component.driverStatus).toBe(
				3,
				`xtuService is ${component.gamingCapabilities.xtuService}, nvDriver is ${component.gamingCapabilities.nvDriver}，component.driverStatus should be 3`
			);
			component.gamingCapabilities.xtuService = true;
			...
			expect(component.driverStatus).toBe(
				2,
				`xtuService is ${component.gamingCapabilities.xtuService}, nvDriver is ${component.gamingCapabilities.nvDriver}，component.driverStatus should be 2`
			);
			...
			expect(component.driverStatus).toBe(
				1,
				`xtuService is ${component.gamingCapabilities.xtuService}, nvDriver is ${component.gamingCapabilities.nvDriver}，component.driverStatus should be 1`
			);
			component.gamingCapabilities.xtuService = false;
			component.gamingCapabilities.nvDriver = false;
			component.driverStatus = 0;
			...
			expect(component.driverStatus).toBe(
				0,
				`xtuService is ${component.gamingCapabilities.xtuService}, nvDriver is ${component.gamingCapabilities.nvDriver}，component.driverStatus keep 0`
			);
		});
	});
	describe('catch error', () => {
		...
		beforeEach(
			waitForAsync(() => {
				const gamingThermalModeServiceSpy = jasmine.createSpyObj(
		...
		it('getThermalModeSettingStatus catch error', () => {
			try {
				thermalModeService.getThermalModeSettingStatus.and.throwError(
					'getThermalModeSettingStatus error'
				);
				thermalModeService.getAutoSwitchStatus.and.returnValue(Promise.resolve(false));
				gamingOCService.getPerformanceOCSetting.and.returnValue(Promise.resolve(false));
				fixture.detectChanges();
		...
		it('setThermalModeSettingStatus catch error', () => {
			try {
				...
				thermalModeService.getAutoSwitchStatus.and.returnValue(Promise.resolve(false));
				gamingOCService.getPerformanceOCSetting.and.returnValue(Promise.resolve(false));
				thermalModeService.setThermalModeSettingStatus.and.throwError(
					'setThermalModeSettingStatus error'
				);
				fixture.detectChanges();
		...
		it('registerThermalModeChangeEvent catch error', () => {
			try {
				...
				thermalModeService.getAutoSwitchStatus.and.returnValue(Promise.resolve(false));
				gamingOCService.getPerformanceOCSetting.and.returnValue(Promise.resolve(false));
				thermalModeService.regThermalModeChangeEvent.and.throwError(
					'regThermalModeChangeEvent error'
				);
				fixture.detectChanges();
				component.registerThermalModeChangeEvent();
			} catch (err) {
		...
		it('getPerformanceOCSetting catch error', () => {
			try {
				thermalModeService.getThermalModeSettingStatus.and.returnValue(Promise.resolve(2));
				thermalModeService.getAutoSwitchStatus.and.returnValue(Promise.resolve(false));
				gamingOCService.getPerformanceOCSetting.and.throwError(
		...
		it('getAutoAdjustSetting', () => {
			try {
				thermalModeService.getAutoAdjustSetting.and.throwError(
					'getAutoAdjustSetting error'
				);
				component.getAutoAdjustSetting();
			} catch (err) {
		...
		it('setAutoAdjustSetting', () => {
			try {
				thermalModeService.setAutoAdjustSetting.and.throwError(
					'getAutoAdjustSetting error'
				);
				component.autoAdjustSettings = false;
				autoAdjustSettingStatus = false;
				...
				expect(component.autoAdjustSettings).toBe(
					false,
					'component.autoAdjustSettings shoulde keep false'
				);
				expect(autoAdjustSettingStatus).toBe(
					false,
					'autoAdjustSettingStatus shoulde keep false'
				);
				expect(autoAdjustSettingStatusCache).toBe(
					false,
					'autoAdjustSettingStatusCache shoulde keep false'
				);
			}
			try {
				thermalModeService.setAutoAdjustSetting.and.throwError(
				...
				expect(component.autoAdjustSettings).toBe(
					true,
					'component.autoAdjustSettings shoulde keep true'
				);
				expect(autoAdjustSettingStatus).toBe(
					true,
					'autoAdjustSettingStatus shoulde keep true'
				);
				expect(autoAdjustSettingStatusCache).toBe(
					true,
					'autoAdjustSettingStatusCache shoulde keep true'
				);
			}
		});
	});
	describe('about modal', () => {
		...
		it('closeThermalMode2Modal', () => {
			let calledTimes = activeModalService.close.calls.count();
			component.closeThermalMode2Modal();
			expect(activeModalService.close).toHaveBeenCalledTimes(++calledTimes);
