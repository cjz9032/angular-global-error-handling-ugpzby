...
export class ModalSmartStandByComponent implements OnInit, AfterViewInit {
	...
	public items: any = [
		...
		{
			tittle: this.device + 'graph.graph2Tittle',
			subTittle: this.device + 'graph.graphSubtittle',
			legends: '',
		},
	];
	...
	private renderActivityChart(
		...
		// chart plot area
		const chart = svg.append('g').attr('transform', `translate(45 20)`);
		chart
			.selectAll('g')
			.data(hours)
			.enter()
		...
		this.powerService
			...
			.then((data: SmartStandbyActivityModel[]) => {
				if (data && data.length > 0) {
					...
					this.loggerService.info(
						'ModalSmartStandByComponent.getSmartStandbyPresenceData',
						data
					);
				} else {
			...
			.catch((error) => {
				this.loggerService.exception(
					'ModalSmartStandByComponent.getSmartStandbyPresenceData error',
					error
				);
			});
	}
		...
		this.powerService
			...
			.then((data: SmartStandbyActivityModel[]) => {
				if (data && data.length > 0) {
					...
					this.loggerService.info(
						'ModalSmartStandByComponent.getSmartStandbyActiveHours',
						data
					);
				} else {
					this.renderActivityChart(
			...
			.catch((error) => {
				this.loggerService.exception(
					'ModalSmartStandByComponent.getSmartStandbyActiveHours error',
					error
				);
			});
	}
