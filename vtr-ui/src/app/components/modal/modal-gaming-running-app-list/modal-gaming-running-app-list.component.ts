...
export class ModalGamingRunningAppListComponent implements OnInit, OnChanges {
    ...
    async refreshList() {
        try {
            ...
            this.loggerService.error('modal-gaming-running-app-list.component => ERROR in refreshList()', error);
        } finally {
        }
    }
    ...
    focusCloseButton() {
        setTimeout(() => {
            const modal = document.getElementsByClassName('modal-gaming-running-app-list-close')[0] as HTMLElement;
            if (modal) {
                modal.focus();
            }
        }, 20);
