...
export class WifiSecurityService {
	...
	constructor(
		...
		const cacheWifiSecurityHistory = this.localCacheService.getLocalCacheValue(
		...
		if (this.wifiSecurity && this.wifiSecurity.state) {
			if (this.wifiSecurity.isLocationServiceOn !== undefined) {
				this.isLWSEnabled =
					this.wifiSecurity.state === 'enabled' && this.wifiSecurity.isLocationServiceOn;
			}
			this.localCacheService.setLocalCacheValue(
				LocalStorageKey.SecurityWifiSecurityState,
				this.wifiSecurity.state
			);
		} else if (cacheWifiSecurityState) {
			if (this.wifiSecurity && this.wifiSecurity.isLocationServiceOn !== undefined) {
				this.isLWSEnabled =
					cacheWifiSecurityState === 'enabled' && this.wifiSecurity.isLocationServiceOn;
			}
		}
		if (this.wifiSecurity && this.wifiSecurity.wifiHistory) {
			this.localCacheService.setLocalCacheValue(
				LocalStorageKey.SecurityWifiSecurityHistorys,
				this.wifiSecurity.wifiHistory
			);
