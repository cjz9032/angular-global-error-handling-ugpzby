...
export class WifisecurityProxy {
	...
	constructor(originWifiSecurity: any, commonService: CommonService) {
		...
		this.originEnableWifiSecurityFunc = this.wifisecurity.enableWifiSecurity.bind(
			this.wifisecurity
		);
		this.originGetWifiSecurityStateOnce = this.wifisecurity.getWifiSecurityStateOnce.bind(
			this.wifisecurity
		);
		...
		return this.originEnableWifiSecurityFunc().then((ret) => {
			if (ret) {
				this.commonService.sendNotification(
					SecurityAdvisorNotifications.WifiSecurityTurnedOn
				);
			}
			return ret;
		});
	...
	getWifiSecurityStateOnce(): Promise<any> {
		return this.originGetWifiSecurityStateOnce().then((re) => {
			this.wifisecurity.on(
				phoenix.EventTypes.wsIsLocationServiceOnEvent,
				this.wsIsLocationServiceOnEventHandler.bind(this)
			);
			return re;
		});
	}
