...
export class AntivirusService {
	...
	constructor(
		...
		this.cacheAvStatus = this.localCacheService.getLocalCacheValue(
			LocalStorageKey.SecurityLandingAntivirusStatus
		);
		this.cacheFwStatus = this.localCacheService.getLocalCacheValue(
			LocalStorageKey.SecurityLandingAntivirusFirewallStatus
		);
		this.cacheCurrentPage = this.localCacheService.getLocalCacheValue(
			LocalStorageKey.SecurityCurrentPage
		);
		this.cacheFirewallLink = this.localCacheService.getLocalCacheValue(
	...
	public GetAntivirusStatus(): AntivirusCommonData {
		if (this.antivirus.mcafee || this.antivirus.windowsDefender || this.antivirus.others) {
			...
			this.cacheAvStatus !== undefined ||
			this.cacheFwStatus !== undefined ||
			this.cacheCurrentPage ||
			this.cacheFirewallLink
		) {
	...
	private setPage(antiVirus: Antivirus) {
		...
		if (
			...
			if (antiVirus.windowsDefender) {
				...
				this.antivirusCommonData.firewallLink =
					antiVirus.others.firewall.length > 0
						? '/security/anti-virus'
		...
		this.localCacheService.setLocalCacheValue(
			LocalStorageKey.SecurityCurrentPage,
			this.antivirusCommonData.currentPage
		);
		this.localCacheService.setLocalCacheValue(
			LocalStorageKey.SecurityFirewallLink,
			this.antivirusCommonData.firewallLink
