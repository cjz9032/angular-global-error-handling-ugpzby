...
export class ServiceEssentialHelper implements IEssentialHelper {
	...
	constructor(vantageShellService: VantageShellService, private devService: DevService) {
	...
	public async getUpeEssential(): Promise<IUpeEssential> {
		...
		try {
			...
			this.devService.writeLog(
				'[service essential helper]get upe essential from upe agent failed',
				ex
			);
		}
	...
	public async registerDevice(essential: IUpeEssential): Promise<IUpeEssential> {
		...
		try {
			...
			this.devService.writeLog(
				'[service essential helper]register device from upe agent failed',
				ex
			);
		}
		return iUpeEssential;
