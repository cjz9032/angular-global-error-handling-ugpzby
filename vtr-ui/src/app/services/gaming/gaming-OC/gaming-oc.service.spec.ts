	...
	describe('isShellAvailable is true', () => {
		...
		it('set performanceOCSetting should return boolean value', fakeAsync(() => {
			...
			gamingOCService.setPerformanceOCSetting(true).then((res) => {
				...
				expect(performanceOCSetting).toBe(
					true,
					`setReturnValue is ${setReturnValue}, set performanceOCSetting to be true FAIL`
				);
			});
			tick();
			performanceOCSetting = true;
			gamingOCService.setPerformanceOCSetting(false).then((res) => {
				...
				expect(performanceOCSetting).toBe(
					false,
					`setReturnValue is ${setReturnValue}, set performanceOCSetting to be false FAIL`
				);
			});
			tick();
			...
			gamingOCService.setPerformanceOCSetting(true).then((res) => {
				...
				expect(performanceOCSetting).toBe(
					false,
					`setReturnValue is ${setReturnValue}, performanceOCSetting should keep false`
				);
			});
			tick();
			performanceOCSetting = true;
			gamingOCService.setPerformanceOCSetting(false).then((res) => {
				...
				expect(performanceOCSetting).toBe(
					true,
					`setReturnValue is ${setReturnValue}, performanceOCSetting should keep true`
				);
			});
		}));
	});
