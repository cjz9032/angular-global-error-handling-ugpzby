	...
	describe('isShellAvailable is true', () => {
			...
			for (let i = 1; i < 4; i++) {
				...
				gamingThermalModeService.setThermalModeSettingStatus(i).then((res) => {
					...
					expect(thermalModeStatus).toBe(
						i,
						`setReturnValue is ${setReturnValue}, set thermal mode status to be ${i} FAIL!`
					);
				});
			}
			tick();
			...
			for (let i = 1; i < 4; i++) {
				...
				for (let j = 1; j < 4; j++) {
					...
					gamingThermalModeService.setThermalModeSettingStatus(j).then((res) => {
						...
						expect(thermalModeStatus).toBe(
							i,
							`setReturnValue is ${setReturnValue}, thermal mode status should keep ${i}`
						);
					});
				}
			}
		...
		it('setAutoSwitchStatus should return undefined', fakeAsync(() => {
			...
			gamingThermalModeService.setAutoSwitchStatus(true).then((res) => {
				expect(res).toBe(
					true,
					`setReturnValue is ${setReturnValue}, setAutoSwitchStatus should return true`
				);
			...
			gamingThermalModeService.setAutoSwitchStatus(true).then((res) => {
				expect(res).toBe(
					false,
					`setReturnValue is ${setReturnValue}, setAutoSwitchStatus should return false`
				);
				expect(autoSwitchStatus).toBe(
					false,
					`setReturnValue is ${setReturnValue}, autoSwitchStatus should be false`
	...
	describe('catch error', () => {
		...
		it('setThermalModeSettingStatus should return err', fakeAsync(() => {
			try {
				...
				expect(err).toMatch(
					'setThermalModeStatus error',
					'set thermal mode status to 1 FAIL'
				);
			}
			tick();
			try {
				...
				expect(err).toMatch(
					'setThermalModeStatus error',
					'set thermal mode status to 2 FAIL'
				);
			}
			tick();
			try {
				...
				expect(err).toMatch(
					'setThermalModeStatus error',
					'set thermal mode status to 3 FAIL'
				);
			}
		}));
		...
		it('setAutoSwitchStatus should return err', fakeAsync(() => {
			try {
				...
				expect(err).toMatch(
					'setAutoSwitchStatus error',
					'set autoSwitch status to true FAIL'
				);
			}
			tick();
			try {
				...
				expect(err).toMatch(
					'setAutoSwitchStatus error',
					'set autoSwitch status to false FAIL'
				);
			}
		}));
