    ...
    describe('isShellAvailable is true', () => {
        ...
        it('set over drive status should return boolean value', fakeAsync(() => {
            ...
            gamingOverDriveService.setOverDriveStatus(true).then( res => {
                ...
                expect(overDriveStatus).toBe(true, `setReturnValue is ${setReturnValue}, set overDriveStatus to be true FAIL`);
            });
            tick();
            overDriveStatus = true;
            gamingOverDriveService.setOverDriveStatus(false).then( res => {
                expect(res).toBe(true, 'setReturnValue should be true');
                expect(overDriveStatus).toBe(false, `setReturnValue is ${setReturnValue}, set overDriveStatus to be false FAIL`);
            });
            tick();
            ...
            gamingOverDriveService.setOverDriveStatus(true).then( res => {
                ...
                expect(overDriveStatus).toBe(false, `setReturnValue is ${setReturnValue}, overDriveStatus should keep false`);
            });
            tick();
            overDriveStatus = true;
            gamingOverDriveService.setOverDriveStatus(false).then( res => {
                expect(res).toBe(false, 'setReturnValue should be false');
                expect(overDriveStatus).toBe(true, `setReturnValue is ${setReturnValue}, overDriveStatus should keep true`);
            });
        }))
    })
