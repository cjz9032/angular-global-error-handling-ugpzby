...
describe('Shared service:', () => {
	...
	describe(':', () => {
		...
		it('should call functions throw error', () => {
			const { service } = setup();
			try{
				spyOn(service.gamingNetworkBoost, 'getStatus').and.throwError(new Error('new getStatus error'));
				service.getNetworkBoostStatus();
			} catch(error) {
			...
			try{
				spyOn(service.gamingNetworkBoost, 'getProcessesInNetworkBoost').and.throwError(new Error('new getProcessesInNetworkBoost error'));
				service.getNetworkBoostList();
			} catch(error) {
				expect(service.gamingNetworkBoost.getProcessesInNetworkBoost).toThrowError('new getProcessesInNetworkBoost error');
			}

			try{
				spyOn(service.gamingNetworkBoost, 'getNetUsingProcesses').and.throwError(new Error('new getNetUsingProcesses error'));
				service.getNetUsingProcesses();
			} catch(error) {
				expect(service.gamingNetworkBoost.getNetUsingProcesses).toThrowError('new getNetUsingProcesses error');
			}

			try{
				spyOn(service.gamingNetworkBoost, 'setStatus').and.throwError(new Error('new setStatus error'));
				service.setNetworkBoostStatus();
			} catch(error) {
			...
			try{
				spyOn(service.gamingNetworkBoost, 'addProcessToNetBoost').and.throwError(new Error('new addProcessToNetBoost error'));
				service.addProcessToNetworkBoost(true);
			} catch(error) {
				expect(service.gamingNetworkBoost.addProcessToNetBoost).toThrowError('new addProcessToNetBoost error');
			}

			try{
				spyOn(service.gamingNetworkBoost, 'deleteProcessInNetBoost').and.throwError(new Error('new deleteProcessInNetBoost error'));
				service.deleteProcessInNetBoost(true);
			} catch(error) {
				expect(service.gamingNetworkBoost.deleteProcessInNetBoost).toThrowError('new deleteProcessInNetBoost error');
			}

			try{
				spyOn(localCache, 'getLocalCacheValue').and.throwError(new Error('new getLocalCacheValue error'));
				service.getNeedToAsk();
			} catch(error) {
			...
			try{
				spyOn(localCache, 'setLocalCacheValue').and.throwError(new Error('new setLocalCacheValue error'));
