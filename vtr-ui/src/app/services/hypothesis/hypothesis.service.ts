...
export class HypothesisService {
	...
	constructor(private shellService: VantageShellService, private devService: DevService) {
				...
				this.getHypothesis().then(
					() => {
						if (this.hypSettings) {
							...
							this.devService.writeLog(
								'get hypothesis  getFeatureSetting: setting failed.'
							);
					...
					(error) => {
						reject(error);
						this.devService.writeLog(
							'get hypothesis  getFeatureSetting: setting failed.' + error
						);
					}
				);
			}
