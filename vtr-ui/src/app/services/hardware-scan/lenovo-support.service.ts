...
export class LenovoSupportService {
  ...
  private isETicketAvailableResponse: Promise<any>;
  ...
  public async startCheckingIfETicketIsAvailable() {
    ...
    this.isETicketAvailableResponse = this.isETicketAvailable(machineSerialNumber)
  }

  public async getSupportUrl(scanDate: Date): Promise<string> {
    try {
      const response = await this.isETicketAvailableResponse;
