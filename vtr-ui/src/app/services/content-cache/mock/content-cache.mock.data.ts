...
export class MockContentLocalCacheTest {
	...
	private getContents(par) {
		if (par.Key === '' + Md5.hashStr(JSON.stringify({ Page: 'normalContents' }))) {
			...
			par.Key ===
			'' + Md5.hashStr(JSON.stringify({ Page: 'expiredDateWithPoistionB' }))
		) {
			par.Value = JSON.stringify(EXPIRED_DATE_INPOISTIONB);
		} else if (
			par.Key ===
			'' + Md5.hashStr(JSON.stringify({ Page: 'NotReachDisplayDateWithPoistionB' }))
		) {
			par.Value = JSON.stringify(DISPALY_DATE_INPOISTIONB);
		} else if (
			par.Key ===
			'' + Md5.hashStr(JSON.stringify({ Page: 'multi-ItemsInPoistionB' }))
		) {
			par.Value = JSON.stringify(MULITI_ITEM_INPOISTIONB);
		} else if (
			par.Key ===
			'' + Md5.hashStr(JSON.stringify({ Page: 'noOnlineContent', Lang: 'en' }))
		) {
			par.Value = null;
